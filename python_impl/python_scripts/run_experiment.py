...
DEFAULT_OUTPUT_JSON = ROOT_DIR / "python_scripts" / "last_run_summary.json"
...
def main() -> None:
    ...
    for alg_name in selected_algorithms:
        ...
        nr[alg_name] = _compute_nr_db(d, results["err_hist"], args.fs)

        print(f"{alg_name} 仿真耗时 {dt:.6f} 秒。")
        print(f"{alg_name} 末1秒平均NR(d/e): {float(np.mean(nr[alg_name])):.4f} dB")
    ...
    if not out_path.is_absolute():
        # Keep relative paths intuitive: resolve against current working directory.
