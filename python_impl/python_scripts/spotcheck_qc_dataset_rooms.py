...
def _build_diagonal_weight_tensor(w_opt: np.ndarray, filter_len: int, n_ref: int, n_sec: int) -> np.ndarray:
...
def main() -> None:
    ...
    with h5py.File(str(h5_path), "r") as f:
        ...
        for idx in indices:
            ...
            noise, t = wn_gen(fs=fs, duration=duration_s, f_low=f_low, f_high=f_high, rng=np.random.default_rng(args.seed + idx))
            ...
            w_diag = _build_diagonal_weight_tensor(w_opt, int(args.filter_len), x.shape[1], len(SEC_IDS))
            e_fixed = _simulate_error_with_fixed_w(mgr, x, d, w_diag, SEC_IDS, ERR_IDS)
