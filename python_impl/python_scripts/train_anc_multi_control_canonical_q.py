...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the canonical-Q multi-control ANC model.")
...
def build_dataset(feats: dict[str, np.ndarray], bundle, idx: np.ndarray, variant: str) -> TensorDataset:
    tensors: list[torch.Tensor] = [
        ...
        torch.from_numpy(feats["d_paths"][idx]),
        torch.from_numpy(feats["s_matrix"][idx]),
    ]
    if "xref" in variant:
...
def unpack_batch(batch: tuple[torch.Tensor, ...], variant: str, device: torch.device) -> tuple[dict[str, torch.Tensor], dict[str, torch.Tensor]]:
    ...
    inputs = {
        ...
        "d_paths": batch[pos + 1].to(device=device, dtype=torch.float32, non_blocking=True),
        "s_matrix": batch[pos + 2].to(device=device, dtype=torch.float32, non_blocking=True),
    }
    pos += 3
    if "xref" in variant:
...
def reconstruct_q_multi_torch(anchor: torch.Tensor, local: torch.Tensor, tail: torch.Tensor, basis_t: torch.Tensor, tap_len: int, half_width: int) -> torch.Tensor:
    flat = reconstruct_q_torch(anchor.reshape(-1), local.reshape(-1, local.shape[-1]), tail.reshape(-1, tail.shape[-1]), basis_t, tap_len=int(tap_len), half_width=int(half_width))
...
def evaluate_epoch(loader: DataLoader, model: torch.nn.Module, variant: str, device: torch.device, basis_t: torch.Tensor, meta: dict[str, Any], target_stats: dict[str, np.ndarray], loss_weights: dict[str, float], label_smoothing: float) -> dict[str, float]:
    model.eval()
    local_mean_t = torch.from_numpy(np.asarray(target_stats["local_mean"], dtype=np.float32)).to(device=device)[None, None, :]
    local_std_t = torch.from_numpy(np.asarray(target_stats["local_std"], dtype=np.float32)).to(device=device)[None, None, :]
    tail_mean_t = torch.from_numpy(np.asarray(target_stats["tail_mean"], dtype=np.float32)).to(device=device)[None, None, :]
    tail_std_t = torch.from_numpy(np.asarray(target_stats["tail_std"], dtype=np.float32)).to(device=device)[None, None, :]
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            q_teacher = reconstruct_q_multi_torch(targets["anchor"], local_pred, tail_pred, basis_t, tap_len=int(meta["q_keep_len"]), half_width=int(meta["local_half_width"]))
            local_true_norm = (targets["local"] - local_mean_t) / local_std_t
            tail_true_norm = (targets["tail"] - tail_mean_t) / tail_std_t
            anchor_loss = F.cross_entropy(outputs["anchor_logits"].reshape(-1, outputs["anchor_logits"].shape[-1]), targets["anchor"].reshape(-1), label_smoothing=float(label_smoothing))
            ...
            pred_q = reconstruct_q_multi_torch(pred_anchor, local_pred, tail_pred, basis_t, tap_len=int(meta["q_keep_len"]), half_width=int(meta["local_half_width"]))
            bs = int(targets["anchor"].shape[0])
            loss_sum += float(total.detach().cpu()) * bs
            q_mse_sum += float(F.mse_loss(pred_q, targets["q"]).detach().cpu()) * bs
            count += bs
            pred_anchor_all.append(pred_anchor.cpu().numpy().reshape(-1))
            true_anchor_all.append(targets["anchor"].cpu().numpy().reshape(-1))
            pred_q_all.append(pred_q.cpu().numpy().reshape(-1, pred_q.shape[-1]))
            true_q_all.append(targets["q"].cpu().numpy().reshape(-1, targets["q"].shape[-1]))
...
def predict_q(model: torch.nn.Module, variant: str, feats: dict[str, np.ndarray], idx: list[int] | np.ndarray, device: torch.device, basis: np.ndarray, meta: dict[str, Any], target_stats: dict[str, np.ndarray]) -> tuple[np.ndarray, np.ndarray]:
    index = np.asarray(idx, dtype=np.int64)
    model.eval()
    with torch.no_grad():
        outputs = model(
            p_ref=torch.from_numpy(feats["p_ref"][index]).to(device=device, dtype=torch.float32),
            d_paths=torch.from_numpy(feats["d_paths"][index]).to(device=device, dtype=torch.float32),
            s_matrix=torch.from_numpy(feats["s_matrix"][index]).to(device=device, dtype=torch.float32),
...
def main() -> int:
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        for batch in train_loader:
            ...
            local_pred = outputs["local_kernel"] * local_std_t + local_mean_t
            tail_pred = outputs["tail_coeffs"] * tail_std_t + tail_mean_t
            q_teacher = reconstruct_q_multi_torch(targets["anchor"], local_pred, tail_pred, basis_t, tap_len=int(bundle.meta["q_keep_len"]), half_width=int(bundle.meta["local_half_width"]))
            local_true_norm = (targets["local"] - local_mean_t) / local_std_t
            tail_true_norm = (targets["tail"] - tail_mean_t) / tail_std_t
            anchor_loss = F.cross_entropy(outputs["anchor_logits"].reshape(-1, outputs["anchor_logits"].shape[-1]), targets["anchor"].reshape(-1), label_smoothing=float(args.label_smoothing))
            local_loss = 0.5 * (F.l1_loss(outputs["local_kernel"], local_true_norm) + F.mse_loss(outputs["local_kernel"], local_true_norm))
            tail_loss = F.mse_loss(outputs["tail_coeffs"], tail_true_norm)
            q_loss = F.mse_loss(q_teacher, targets["q"]) / float(q_scale)
            total = (
        ...
        if epoch == 1 or epoch % int(args.replay_eval_interval) == 0 or epoch == int(args.epochs):
            ...
            train_replay = summarize_replay(bundle, room_indices=train_probe, q_truncated=q_train_probe, early_window_s=float(args.replay_early_window_s))
            val_replay = summarize_replay(bundle, room_indices=val_probe, q_truncated=q_val_probe, early_window_s=float(args.replay_early_window_s))
            ...
            row["val_probe_plant_residual"] = val_replay["plant_residual_mean"]
