...
def cdfxlms(params: Dict):
    """Distributed CDFxLMS algorithm."""
    time = params["time"]
    ...
    for n in range(n_samples):
        ...
        for node in network.nodes.values():
            ...
            node.direction = e[n, err_idx[node.err_mic_id]] * node.xf_taps
            ...
            for neighbor_id in node.neighbor_ids:
                ...
                s_hat_km = rir_manager.get_secondary_rir(node.sec_spk_id, neighbor.err_mic_id)
                ls_hat_km = len(s_hat_km)
                xf_km = float(s_hat_km @ x_taps[:ls_hat_km, ref_col])

                buffer = node.xf_taps_km.get(neighbor_id)
                if buffer is None:
                    buffer = np.zeros(l, dtype=float)

                buffer = np.concatenate(([xf_km], buffer[:-1]))
                node.xf_taps_km[neighbor_id] = buffer

                node.direction = node.direction + e[n, err_idx[neighbor.err_mic_id]] * buffer
