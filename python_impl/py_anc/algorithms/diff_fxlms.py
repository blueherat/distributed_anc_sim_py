...
def diff_fxlms(params: Dict):
    """Distributed diffusion FxLMS algorithm."""
    time = params["time"]
    ...
    for n in range(n_samples):
        ...
        for node in network.nodes.values():
            node.psi = node.w - node.step_size * e[n, err_idx[node.err_mic_id]] * node.xf_taps

        for node in network.nodes.values():
            w_new = np.zeros(l, dtype=float)
            for neighbor_id in node.neighbor_ids:
                neighbor = network.nodes[neighbor_id]
                w_new += neighbor.psi / len(node.neighbor_ids)
            node.w = w_new
