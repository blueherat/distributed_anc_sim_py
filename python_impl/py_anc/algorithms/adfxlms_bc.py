...
def adfxlms_bc(params: Dict):
    """Distributed ADFxLMS-BC algorithm."""
    time = params["time"]
    ...
    for n in range(n_samples):
        ...
        for node in network.nodes.values():
            self_col = node.neighbor_ids.index(node.node_id)
            node.phi[:, self_col] = 0.0
            for neighbor_id in node.neighbor_ids:
                neighbor = network.nodes[neighbor_id]
                idx_in_neighbor = neighbor.neighbor_ids.index(node.node_id)
                node.phi[:, self_col] += neighbor.psi[:, idx_in_neighbor] / len(node.neighbor_ids)

        for node in network.nodes.values():
            for neighbor_id in node.neighbor_ids:
                neighbor = network.nodes[neighbor_id]
                idx_node_neighbor = node.neighbor_ids.index(neighbor_id)
                idx_neighbor_neighbor = neighbor.neighbor_ids.index(neighbor_id)
                node.phi[:, idx_node_neighbor] = neighbor.phi[:, idx_neighbor_neighbor]
