...
def main() -> int:
    ...
    stage_cfg = resolve_stage_level("05", args.level, num_samples=args.num_samples)
    output_h5 = args.output_h5 or str(Path(__file__).resolve().parents[1] / "data" / f"single_control_relative_anechoic_{stage_cfg['level'].lower()}.h5")
    cfg = SingleControlValidationConfig(num_samples=int(stage_cfg["num_samples"]), seed=int(args.seed), profile="anechoic")
