...
def main() -> int:
    ...
    stage_cfg = resolve_stage_level("03", args.level)
    h5_path = args.h5_path or str(Path(__file__).resolve().parents[1] / "data" / f"single_control_anechoic_{stage_cfg['level'].lower()}.h5")
    output_dir = args.output_dir or str(Path(__file__).resolve().parents[1] / "results" / stage_cfg["level"])
    print(evaluate_single_control_suite(h5_path, output_dir, feature_kind="positions", device=str(args.device), stage_id="03", level=str(stage_cfg["level"])))
    return 0
